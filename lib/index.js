...
export default function slackin({
  ...
  let channels;
  if (channelFilter) {
    channels = channelFilter.split(',').map(channel => (
  ...
  app.post('/invite', json(), (req, res) => {
    ...
    } else if (!arrayContainsArray(channels, channel)) {
    ...
    const captchaCallback = (err) => {
      ...
      return invite({
        token, org, email, logger: inviteLog, channel: slack.getChannelId(channel),
      }, (inviteErr) => {
        if (inviteErr) {
